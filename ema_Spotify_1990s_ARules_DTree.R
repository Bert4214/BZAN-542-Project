...
spotify_1990s = read.csv("spotify_1990s.csv")

## Data Prep
spotify_1990s1 = spotify_1990s[,-c(1:4,8:10,24:25)]
spotify1 = spotify_1990s1
...
## DECISION TREE
#Combos
TREE <- rpart(popularity ~ .,data=spotify1,cp= 0.0003598417  )
TREE$cptable
summarize_tree(TREE)
...
RULES <- apriori(SPOTIFY.TRANS,parameter = list(supp=200/length(SPOTIFY.TRANS),conf=0.032,maxlen=4),
...
itemFrequency(SPOTIFY.TRANS)["popularity=High"]
#Consider pockets of size 200 or more
RULES <- apriori(SPOTIFY.TRANS,parameter = list(supp=200/length(SPOTIFY.TRANS),conf=0.20,maxlen=4),
                 appearance = list(default="lhs",rhs="popularity=High"), control=list(verbose=FALSE))
RULES <- RULES[!is.redundant(RULES)]
RULES <- RULES[is.significant(RULES,SPOTIFY.TRANS)]
length(RULES)
