...
## Decision Tree and Association Rules for 1980s Songs
spotify_1980s = read.csv("spotify_1980s.csv")

mean(spotify_1980s$popularity)


## Data Prep
spotify_1980s1 = spotify_1980s[,-c(1:4,8:10,24:25)]
spotify1 = spotify_1980s1
spotify1 = spotify1[complete.cases(spotify1),]

table(spotify1$popularity)

###

summary_df_1980 <- spotify_1980s %>% 
  group_by(year) %>% 
...
mean(summary_df_1980$AvgFollow)
...
## DECISION TREE
#Combos
TREE <- rpart(popularity ~ .,data=spotify1,cp= 0.0002149152)
TREE$cptable
summarize_tree(TREE)
...
RULES <- apriori(SPOTIFY.TRANS,parameter = list(supp=200/length(SPOTIFY.TRANS),conf=0.005,maxlen=4),
